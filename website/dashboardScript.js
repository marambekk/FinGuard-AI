...
(async function checkAuth() {
    ...
    if (!token) {
        ...
        setTimeout(() => window.location.href = "SignInPage.html", 1500);
        return;
    }
...
async function fetchDashboardData(token) {
    try {
        ...
        try {
                ...
                if (!iocTxnData || iocTxnData.length === 0) {
                    ...
                    iocTxnData.forEach(txn => {
                        ...
                        row.innerHTML = `
                            ...
                            <td>${renderFlag(txn.high_risk_ip)}</td>
                            <td>${renderFlag(txn.disposable_email)}</td>
                            <td>${renderFlag(txn.device_velocity)}</td>
                            <td>${renderFlag(txn.pii_changes)}</td>
            ...
            console.warn("Could not load transactions with IOC flags:", err);
        }
        ...
        =========================== */
        try {
            const iocScoreResp = await fetch(`${API_BASE_URL}/dashboard/ioc-score-ranking`, { headers });
            const iocScoreData = await iocScoreResp.json();
            const iocScoreTable = document.querySelector("#ioc-score-ranking-table tbody");
            if (iocScoreTable) {
                iocScoreTable.innerHTML = "";
                if (!iocScoreData || iocScoreData.length === 0) {
                    iocScoreTable.innerHTML = "<tr><td colspan='6'>No IOC score data</td></tr>";
                } else {
                    iocScoreData.forEach(txn => {
                        const row = document.createElement("tr");
                        row.innerHTML = `
                            <td>${txn.transaction_id || "-"}</td>
                            <td>${txn.user_id || "-"}</td>
                            ...
                            <td>${txn.system_action || "-"}</td>
                            <td>${(Number(txn.ioc_score) || 0).toFixed(2)}</td>
                        `;
                        iocScoreTable.appendChild(row);
                    });
                }
            }
        } catch (err) {
            console.warn("Could not load IOC score ranking:", err);
        }
